...
public class Servlets {
	 ...
	 * 设置客户端缓存过期时间 的Header.
	 */
	public static void setExpiresHeader(final HttpServletResponse response,
			final long expiresSeconds) {
	 ...
	 */
	public static void setLastModifiedHeader(
			final HttpServletResponse response, final long lastModifiedDate) {
	 ...
	 * 设置Etag Header.
	 */
	public static void setEtag(final HttpServletResponse response,
			final String etag) {
	...
	public static boolean checkIfModifiedSince(
			...
			final HttpServletResponse response, final long lastModified) {
		final long ifModifiedSince = request
	...
	public static boolean checkIfNoneMatchEtag(
			final HttpServletRequest request,
			final HttpServletResponse response, final String etag) {
		final String headerValue = request.getHeader(HttpHeaders.IF_NONE_MATCH);
		if (headerValue != null) {
			boolean conditionSatisfied = false;
			if (!"*".equals(headerValue)) {
				final StringTokenizer commaTokenizer = new StringTokenizer(
						headerValue, ",");
	...
	public static void setFileDownloadHeader(
		...
		try {
			// 中文文件名支持
			final String encodedfileName = new String(fileName.getBytes(),
					"ISO8859-1");
	...
	public static String encodeParameterStringWithPrefix(
			final Map<String, Object> params, String prefix) {
	 ...
	 */
	public static String encodeHttpBasic(final String userName,
